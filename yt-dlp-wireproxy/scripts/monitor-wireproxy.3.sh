#!/usr/bin/env bash
# monitor-wireproxy.3.sh - detail monitor untuk 3 wireproxy container.
#
...
#
# Tabel dirender dengan python helper (monitor-wireproxy-render.3.py) berwarna,
# dikelompokkan per negara, refresh-able.
#
# Usage:
#   ./monitor-wireproxy.3.sh                 # watch mode, refresh 5 detik
#   ./monitor-wireproxy.3.sh --once          # snapshot 1x lalu exit
#   ./monitor-wireproxy.3.sh --interval 3    # watch mode, refresh 3 detik
#   ./monitor-wireproxy.3.sh --no-color      # nonaktifkan ANSI (untuk pipe ke log)
#   ./monitor-wireproxy.3.sh --help

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
RENDER_PY="${SCRIPT_DIR}/monitor-wireproxy-render.3.py"

GATEWAY_CONTAINER="${GATEWAY_CONTAINER:-ytdlp-gateway-wireproxy-3}"
SOCKS_PROBE_CONTAINER="${SOCKS_PROBE_CONTAINER:-ytdlp-worker-1}"
REDIS_CONTAINER="${REDIS_CONTAINER:-ytdlp-redis-wireproxy-3}"
GATEWAY_PORT="${GATEWAY_PORT:-9111}"
PROXY_COUNT="${PROXY_COUNT:-3}"
...
if ! docker ps --filter "name=^/${GATEWAY_CONTAINER}$" --format '{{.Names}}' \
  ...
  echo "       jalankan: docker compose -f ${PROJECT_DIR}/docker-compose.wireproxy.3.yml up -d" >&2
  exit 1
fi
