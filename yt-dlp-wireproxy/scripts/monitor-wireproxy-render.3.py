#!/usr/bin/env python3
"""monitor-wireproxy-render.3.py - render detail table for 3 wireproxy containers.
...
COUNTRY_ORDER = {"Taiwan": 0, "Indonesia": 1, "Singapore": 2}
...
def main() -> int:
    # usage: monitor-wireproxy-render.3.py <cycle-dir> [--count N]
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        print("usage: monitor-wireproxy-render.3.py <cycle-dir> [--count N]", file=sys.stderr)
        return 2
...
def render(health: dict, rows: list, workers: list, proxy_count: int = 3) -> None:
